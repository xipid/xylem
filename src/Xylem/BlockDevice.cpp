...
bool BlockDevice::writeBlock(u32 blockIdx, u16 eraseCount, const String &data) {
  ...
  for (u32 chunkOff = 0; chunkOff < finalData.size();
       chunkOff += config.writeSize) {
    ...
    if (chunkOff == 0 && chunkLen == finalData.size()) {
      ...
      if (!config.onDeviceWrite(
              offset + chunkOff,
              String(finalData.data() + chunkOff, chunkLen))) {
        return false;
      }
    }
  }
...
String BlockDevice::readBlock(u32 blockIdx, u16 eraseCount) {
  ...
  for (u32 chunkOff = 0; chunkOff < config.blockSize;
       chunkOff += config.readSize) {
    u32 chunkLen = (chunkOff + config.readSize > config.blockSize)
                       ? (config.blockSize - chunkOff)
                       : config.readSize;

    String chunk =
        config.onDeviceRead(offset + chunkOff, offset + chunkOff + chunkLen);
