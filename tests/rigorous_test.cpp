...
void setupEngine(XylemEngine &xm) {
  ...
  unlink("rigorous.xy");
  int fd = open("rigorous.xy", O_RDWR | O_CREAT, 0644);
...
int main() {
  ...
  if (res1.size() != 0)
    Error("Test 1 Failed: Returned data for missing column.");
  else
    Success("Test 1 Passed: Handled missing column gracefully.");
  ...
  if (res2.size() != 0)
    Error("Test 2 Failed: Rolled back data was found!");
  else
    Success("Test 2 Passed: Rollback successfully purged data.");

  // --- TEST 3: The 'Oops, Wrong Dimension' Vector Attack ---
  Info("Test 3: Malformed Vector Embedding Attack");
  String badVec;
  badVec.allocate(127 * sizeof(f32)); // Deliberately wrong size! (127 vs 128)
  Array<Clause> r3;
  r3.push({"v_id", "=", "bad"});
  ...
  String goodQuery;
  goodQuery.allocate(128 * sizeof(f32)); // 128 dim query
  // Querying with 128 dims against a 127 dim vector should NOT segfault, it
  // should safely return 0 results.
  auto res3 = xm.read(emptyCols, OR(WHERE("embedding", "cos", goodQuery)));
  Success("Test 3 Passed: Engine survived malformed vector dimensions without "
          "segfaulting.");
  ...
  Success("Test 4 Passed: Watcher survived 10,000 events and mid-stream "
          "unsubscribe.");
  ...
  String dummyQ;
  dummyQ.allocate(dim * sizeof(f32));
  xm.read(emptyCols, OR(WHERE("embedding", "cos", dummyQ)), 5);
  Info("Graph built with 1000 vectors.");

  // Now, NUKE the entire graph instantly. If graph pointers aren't cleaned up,
  // this segfaults on next query.
  xm.remove(OR(WHERE("type", "=", "vector")));

  // Query the empty graph
  auto res5 = xm.read(emptyCols, OR(WHERE("embedding", "cos", dummyQ)), 5);
  if (res5.size() != 0)
    Error("Test 5 Failed: Graph returned deleted vectors.");
  else
    Success(
        "Test 5 Passed: Graph survived total collapse and returned 0 results.");
  ...
  if (blobSz == 51200)
    Success("Test 6 Passed: 100 duplicated 50KB blobs safely resolved to 1 "
            "physical block.");
  else
    Error("Test 6 Failed: CAS Deduplication failed. Size=" +
          String::from((u64)blobSz) +
          " Block=" + String::from((u64)xm.blobStore->index.size()));
