...
int main() {
    ...
    unlink("db.xlm"); // Ensure clean state across test runs
    int fd = open("db.xlm", O_RDWR | O_CREAT, 0644);
    ...
    xm.format();
    xm.mount();
    Success("Engine Mounted. `db.xlm` physical device backing initialized.");

    // Test 2: Rigorous Table CRUD (1000 inserts)
    Info("Test 2: Rigorous Table CRUD (Mass Insert)");
    
    u64 cbWatchId = xm.watch(OR(WHERE("type", "=", "transaction")), [](Map<String, String> row) {
    ...
    Info("Found transaction records: " + String::from((u64)resultTx.size()) + " (Expected: 250)");

    auto resultFailed = xm.read(cols, OR(WHERE("status", "=", "failed")));
    Info("Found failed records: " + String::from((u64)resultFailed.size()) + " (Expected: 100)");

    // Test 3: Watch/Pull Reactivity
    auto events = xm.pull(pullWatchId);
    Info("Watcher intercepted events: " + String::from((u64)events.size()) + " (Expected: 250)");

    // Test 4: Tombstones (Remove)
    Info("Test 4: Tombstones & Deletion");
    Array<Clause> upRow;
    upRow.push({"status", "=", "recovered"});
    xm.write(upRow, OR(WHERE("status", "=", "failed")), 0, "RIGOROUS_SECURE_TEST_KEY_32BYTES");
    
    auto resultRec = xm.read(cols, OR(WHERE("status", "=", "recovered")));
    auto resultFailedAfter = xm.read(cols, OR(WHERE("status", "=", "failed")));
    Info("Failed records after deletion: " + String::from((u64)resultFailedAfter.size()) + " (Expected: 0)");
    ...
    Info("Assert pipeline correctly failed the query. Rows returned: " + String::from((u64)resultAssert.size()) + " (Expected: 0)");
    ...
    for(int i=0; i<100*1024; ++i) massivePayload[i] = 'Z';
    ...
    if (readBack == customData) Success("writeHash(content, hash) works correctly.");
    else Error("writeHash(content, hash) failed!");
    ...
    Info("Virtual commands received: " + String::from((u64)cmds.size()) + " (Expected: 1)");
    
    // Verify virtual columns were NOT stored
    auto virtualCheck = xm.read(cols, OR(WHERE("_type", "=", "command")));
    if (virtualCheck.size() == 0) Success("Virtual columns correctly not stored in DB.");
    else Error("Virtual columns were incorrectly stored!");

    // Test 7.1: :blob column type (transparent storage)
    Info("Test 7.1: Blob Column Type (Transparent Storage)");
    String blobContent = "This is a large blob content that gets hashed and stored via BlobStore transparently.";
    ...
    Array<String> blobCols; blobCols.push("content"); blobCols.push("doc_id");
    ...
    if (blobResult.size() == 1 && *blobResult[0].get("content") == blobContent) {
        ...
        Error(":blob column failed! Got: " + (blobResult.size() > 0 && blobResult[0].has("content") ? *blobResult[0].get("content") : "EMPTY"));
    }
    ...
    if (blobResult.size() == 1 && *blobResult[0].get("content") == expectedAppend) {
    ...
    Array<String> readCols; readCols.push("profile");
    ...
    Info(String("YAML parsed profile name: ") + (nameItem ? nameItem->value : "MISSING"));

    // Test 9: Pub/Sub
    Info("Test 9: Pub/Sub");
    u64 sub = xm.watch(
        OR(WHERE("topic", "=", "events")),
    ...
    usz dim = 128;
    Info("Ingesting " + String::from((u64)numVectors) + " vectors of " + String::from((u64)dim) + " dimensions...");
    
    // Generate a random target vector
    String targetVecStr; targetVecStr.allocate(dim * sizeof(f32));
    f32* targetVec = reinterpret_cast<f32*>(targetVecStr.data());
    for(usz j=0; j<dim; ++j) targetVec[j] = (f32)(Xi::randomNext() % 1000) / 1000.0f - 0.5f;
    ...
    for (usz i = 0; i < numVectors; ++i) {
        ...
        if (i == 7777) {
            for(usz j=0; j<dim; ++j) vec[j] = targetVec[j]; // Perfect Match
        }
        if (i == 8888) {
            for(usz j=0; j<dim; ++j) vec[j] = targetVec[j] + 0.001f; // Close Match
        }
    ...
    Info("Write completed in " + String::from(tIngest) + " ms (lazy ingestion, no graph ops).");
    
    struct stat st;
    stat("db.xlm", &st);
    Info("Physical db.xlm payload size: " + String::from((u64)st.st_size) + " bytes (ZSTD Compression + Encryption Active)");
    
    Info("Executing Top-5 Cosine Similarity Search (triggers on-demand ingestion)...");
    tStart = Xi::millis();
    
    Array<String> retCols; retCols.push("v_id");
    auto topRows = xm.read(retCols, OR(WHERE("embedding", "cos", targetVecStr)), 5);
    
    u64 tSearch = Xi::millis() - tStart;
    Info("On-Demand Ingestion + HNSW Graph Search completed in " + String::from(tSearch) + " ms.");
    
    for (usz i = 0; i < topRows.size(); ++i) {
        Info(" - Rank " + String::from((u64)(i+1)) + ": ID " + topRows[i]["v_id"]);
    }
    ...
    tStart = Xi::millis();
    auto rebootRows = xm.read(retCols, OR(WHERE("embedding", "cos", targetVecStr)), 5);
    u64 tRebootSearch = Xi::millis() - tStart;
    
    Info("Reboot HNSW Search completed in " + String::from(tRebootSearch) + " ms.");
    for (usz i = 0; i < rebootRows.size(); ++i) {
        Info(" - Rank " + String::from((u64)(i+1)) + ": ID " + rebootRows[i]["v_id"]);
    }
    ...
    Array<Clause> rRoot; rRoot.push({"id", "=", "root_id"}); rRoot.push({"type", "=", "dir"}); rRoot.push({"name", "=", "root"});
    Array<Clause> rDir1; rDir1.push({"id", "=", "dir1_id"}); rDir1.push({"parent_id", "=", "root_id"}); rDir1.push({"type", "=", "dir"}); rDir1.push({"name", "=", "dir1"});
    Array<Clause> rDir2; rDir2.push({"id", "=", "dir2_id"}); rDir2.push({"parent_id", "=", "dir1_id"}); rDir2.push({"type", "=", "dir"}); rDir2.push({"name", "=", "dir2"});
    Array<Clause> rFile1; rFile1.push({"id", "=", "f1_id"}); rFile1.push({"parent_id", "=", "dir2_id"}); rFile1.push({"type", "=", "file"}); rFile1.push({"name", "=", "f1.txt"});
    Array<Clause> rFile2; rFile2.push({"id", "=", "f2_id"}); rFile2.push({"parent_id", "=", "dir1_id"}); rFile2.push({"type", "=", "file"}); rFile2.push({"name", "=", "f2.txt"});
    
    xm.write(rRoot); xm.write(rDir1); xm.write(rDir2); xm.write(rFile1); xm.write(rFile2);
    
    // Perform bulk update!
    xm.graphWrite(GRAPH(
        MATCH(WHERE("id", "=", "root_id")),
        SET(WHERE("mod_time", "=", "updated_at_root")),
        REPEATFOLLOW(WHERE("parent_id", "=", "parent.id"), WHERE("type", "=", "file")), // Stop at files
        SET(WHERE("mod_time", "=", "updated_at_dir")), // Updates all dirs under root
        FOLLOW(WHERE("type", "=", "file")),
        SET(WHERE("content", "=", "cleared")) // Update only the files
    ));
    
    auto files = xm.read(Array<String>(), OR(WHERE("type", "=", "file")));
    if (files.size() != 2 || *files[0].get("content") != "cleared") Error("Graph Write failed to update files!");
    else Success("Graph Write successfully updated files atomically!");
    
    // --- Test 13: Graph Traversal (Read) ---
    Info("Test 13: Graph Traversal (Read to XiC Tree)");
    Collection::TreeBranch* tree = xm.graphRead(Array<String>(), GRAPH(
        MATCH(WHERE("id", "=", "root_id")),
        REPEATFOLLOW(WHERE("parent_id", "=", "parent.id"))
    ));
    
    if (!tree || tree->size() != 1) Error("Graph Read failed to find root!");
    else {
        Collection::TreeBranch* rootNode = dynamic_cast<Collection::TreeBranch*>((*tree)[0]);
        if (!rootNode || rootNode->size() != 1) Error("Graph Read failed to populate children!");
        else Success("Graph Read returned a valid XiC TreeBranch structure spanning to the deepest nodes!");
    }
    delete tree;
    ...
    if (!fixResult) Error("Failed to pin raw block!");
    else {
    ...
    if (!mountSuccess) Error("Failed to mount after dynamic system relocation!");
        ...
        if (q != "Hello World, this is a large raw file payload!") {
            Error("Data corrupted after relocation! Length: " + String::from((u64)q.size()) + " Content: " + q.slice(0, 50));
        } else Success("Data fully verified after relocation!");
    }
    ...
    if (mvccResult == -2) Success("MVCC correctly detected write-write conflict and auto-rolled back!");
    else Error("MVCC conflict detection failed. Result: " + String::from((u64)mvccResult));
    ...
    int assertResult = xm.write(
        assertRow,
        OR(ASSERT_WHERE("mvcc_id", "=", "shared"))
    );
    if (assertResult > 0) Success("Write correctly returned ASSERT clause index: " + String::from((u64)assertResult));
    else Error("Write ASSERT returned: " + String::from((u64)assertResult));
    ...
    if (vRes.size() == 1) Success("Volatile row successfully read from memory!");
